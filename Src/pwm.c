...
void pwm_Init(uint32_t pin, TIM_TypeDef* timer, GPIO_TypeDef* port, uint32_t prescaler, uint32_t period, uint32_t pulse, uint32_t channel,TIM_HandleTypeDef *htim){
  TIM_Init(prescaler, period, pulse, channel,htim,pin,timer,port);
